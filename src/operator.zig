const SmallString = @import("string.zig").Small;

const std = @import("std");

pub const Operator = enum {
    none,
    new_generic,
    increment,
    decrement,
    assign,
    equals,
    less_than,
    less_equal,
    greater_than,
    greater_equal,
    plus,
    plus_assign,
    minus,
    minus_assign,
    multiply,
    multiply_assign,
    exponentiate1,
    exponentiate1_assign,
    exponentiate,
    exponentiate_assign,
    divide,
    divide_assign,
    integer_divide,
    integer_divide_assign,
    modulus,
    modulus_assign,
    remainder,
    remainder_assign,
    nullify,
    nullish_or,
    nullish_or_assign,
    not,
    not_not,
    not_equal,
    declare_generic,
    declare_readonly,
    declare_writable,
    declare_temporary,
    comma,
    logical_and,
    logical_and_assign,
    logical_or,
    logical_or_assign,
    bitwise_and,
    bitwise_and_assign,
    bitwise_or,
    bitwise_or_assign,
    bitwise_xor,
    bitwise_xor_assign,
    bitwise_flip,
    bitshift_left,
    bitshift_left_assign,
    bitshift_right,
    bitshift_right_assign,
    lambda1,
    lambda2,
    lambda3,
    lambda4,
    lambda5,
    lambda6,
    lambda7,
    lambda8,
    indent,
    access,
    readonly_member_access,
    writable_member_access,
    temporary_member_access,
    wr_member_access,
    rw_member_access,
    wt_member_access,
    tw_member_access,
    rt_member_access,
    tr_member_access,
    rwt_member_access,
    wrt_member_access,
    rtw_member_access,
    wtr_member_access,
    trw_member_access,
    twr_member_access,

    /// Returns 0 if `buffer` is an invalid operator, otherwise
    /// the numerical value of the operator (see `SmallString.as64`).
    pub fn init(buffer: []const u8) Self {
        if (buffer.len > 8) {
            return .none;
        }
        const small = SmallString.init(buffer) catch unreachable;
        const value64 = small.big64() catch unreachable;
        return init64(value64);
    }

    pub fn init64(value: u64) Self {
        return switch (value) {
            SmallString.as64("~") => .new_generic,
            SmallString.as64("++") => .increment,
            SmallString.as64("--") => .decrement,
            SmallString.as64("=") => .assign,
            SmallString.as64("==") => .equals,
            SmallString.as64("<") => .less_than,
            SmallString.as64("<=") => .less_equal,
            SmallString.as64(">") => .greater_than,
            SmallString.as64(">=") => .greater_equal,
            SmallString.as64("+") => .plus,
            SmallString.as64("+=") => .plus_assign,
            SmallString.as64("-") => .minus,
            SmallString.as64("-=") => .minus_assign,
            SmallString.as64("*") => .multiply,
            SmallString.as64("*=") => .multiply_assign,
            SmallString.as64("**") => .exponentiate1,
            SmallString.as64("**=") => .exponentiate1_assign,
            SmallString.as64("^") => .exponentiate,
            SmallString.as64("^=") => .exponentiate_assign,
            SmallString.as64("/") => .divide,
            SmallString.as64("/=") => .divide_assign,
            SmallString.as64("//") => .integer_divide,
            SmallString.as64("//=") => .integer_divide_assign,
            SmallString.as64("%") => .modulus,
            SmallString.as64("%=") => .modulus_assign,
            SmallString.as64("%%") => .remainder,
            SmallString.as64("%%=") => .remainder_assign,
            SmallString.as64("?") => .nullify,
            SmallString.as64("??") => .nullish_or,
            SmallString.as64("??=") => .nullish_or_assign,
            SmallString.as64("!") => .not,
            SmallString.as64("!!") => .not_not,
            SmallString.as64("!=") => .not_equal,
            SmallString.as64("`") => .declare_generic,
            SmallString.as64(":") => .declare_readonly,
            SmallString.as64(";") => .declare_writable,
            SmallString.as64(".") => .declare_temporary,
            SmallString.as64(",") => .comma,
            SmallString.as64("&&") => .logical_and,
            SmallString.as64("&&=") => .logical_and_assign,
            SmallString.as64("||") => .logical_or,
            SmallString.as64("||=") => .logical_or_assign,
            SmallString.as64("&") => .bitwise_and,
            SmallString.as64("&=") => .bitwise_and_assign,
            SmallString.as64("|") => .bitwise_or,
            SmallString.as64("|=") => .bitwise_or_assign,
            SmallString.as64("><") => .bitwise_xor,
            SmallString.as64("><=") => .bitwise_xor_assign,
            SmallString.as64("<>") => .bitwise_flip,
            SmallString.as64("<<") => .bitshift_left,
            SmallString.as64("<<=") => .bitshift_left_assign,
            SmallString.as64(">>") => .bitshift_right,
            SmallString.as64(">>=") => .bitshift_right_assign,
            SmallString.as64("$") => .lambda1,
            SmallString.as64("$$") => .lambda2,
            SmallString.as64("$$$") => .lambda3,
            SmallString.as64("$$$$") => .lambda4,
            SmallString.as64("$$$$$") => .lambda5,
            SmallString.as64("$$$$$$") => .lambda6,
            SmallString.as64("$$$$$$$") => .lambda7,
            SmallString.as64("$$$$$$$$") => .lambda8,
            SmallString.as64("\t") => .indent,
            SmallString.as64(" ") => .access,
            SmallString.as64("::") => .readonly_member_access,
            SmallString.as64(";;") => .writable_member_access,
            SmallString.as64("..") => .temporary_member_access,
            SmallString.as64(";:") => .wr_member_access,
            SmallString.as64(":;") => .rw_member_access,
            SmallString.as64(";.") => .wt_member_access,
            SmallString.as64(".;") => .tw_member_access,
            SmallString.as64(":.") => .rt_member_access,
            SmallString.as64(".:") => .tr_member_access,
            SmallString.as64(":;.") => .rwt_member_access,
            SmallString.as64(";:.") => .wrt_member_access,
            SmallString.as64(":.;") => .rtw_member_access,
            SmallString.as64(";.:") => .wtr_member_access,
            SmallString.as64(".:;") => .trw_member_access,
            SmallString.as64(".;:") => .twr_member_access,
            // We also convert some unnecessarily verbose operators.
            SmallString.as64(":=") => .declare_readonly,
            SmallString.as64(";=") => .declare_writable,
            else => .none,
        };
    }

    pub fn printLine(self: Self, writer: anytype) !void {
        try self.print(writer);
        try writer.print("\n", .{});
    }

    pub fn print(self: Self, writer: anytype) !void {
        return try switch (self) {
            .none => writer.print("none", .{}),
            .new_generic => writer.print("new_generic", .{}),
            .increment => writer.print("increment", .{}),
            .decrement => writer.print("decrement", .{}),
            .assign => writer.print("assign", .{}),
            .equals => writer.print("equals", .{}),
            .less_than => writer.print("less_than", .{}),
            .less_equal => writer.print("less_equal", .{}),
            .greater_than => writer.print("greater_than", .{}),
            .greater_equal => writer.print("greater_equal", .{}),
            .plus => writer.print("plus", .{}),
            .plus_assign => writer.print("plus_assign", .{}),
            .minus => writer.print("minus", .{}),
            .minus_assign => writer.print("minus_assign", .{}),
            .multiply => writer.print("multiply", .{}),
            .multiply_assign => writer.print("multiply_assign", .{}),
            .exponentiate1 => writer.print("exponentiate1", .{}),
            .exponentiate1_assign => writer.print("exponentiate1_assign", .{}),
            .exponentiate => writer.print("exponentiate", .{}),
            .exponentiate_assign => writer.print("exponentiate_assign", .{}),
            .divide => writer.print("divide", .{}),
            .divide_assign => writer.print("divide_assign", .{}),
            .integer_divide => writer.print("integer_divide", .{}),
            .integer_divide_assign => writer.print("integer_divide_assign", .{}),
            .modulus => writer.print("modulus", .{}),
            .modulus_assign => writer.print("modulus_assign", .{}),
            .remainder => writer.print("remainder", .{}),
            .remainder_assign => writer.print("remainder_assign", .{}),
            .nullify => writer.print("nullify", .{}),
            .nullish_or => writer.print("nullish_or", .{}),
            .nullish_or_assign => writer.print("nullish_or_assign", .{}),
            .not => writer.print("not", .{}),
            .not_not => writer.print("not_not", .{}),
            .not_equal => writer.print("not_equal", .{}),
            .declare_generic => writer.print("declare_generic", .{}),
            .declare_readonly => writer.print("declare_readonly", .{}),
            .declare_writable => writer.print("declare_writable", .{}),
            .declare_temporary => writer.print("declare_temporary", .{}),
            .comma => writer.print("comma", .{}),
            .logical_and => writer.print("logical_and", .{}),
            .logical_and_assign => writer.print("logical_and_assign", .{}),
            .logical_or => writer.print("logical_or", .{}),
            .logical_or_assign => writer.print("logical_or_assign", .{}),
            .bitwise_and => writer.print("bitwise_and", .{}),
            .bitwise_and_assign => writer.print("bitwise_and_assign", .{}),
            .bitwise_or => writer.print("bitwise_or", .{}),
            .bitwise_or_assign => writer.print("bitwise_or_assign", .{}),
            .bitwise_xor => writer.print("bitwise_xor", .{}),
            .bitwise_xor_assign => writer.print("bitwise_xor_assign", .{}),
            .bitwise_flip => writer.print("bitwise_flip", .{}),
            .bitshift_left => writer.print("bitshift_left", .{}),
            .bitshift_left_assign => writer.print("bitshift_left_assign", .{}),
            .bitshift_right => writer.print("bitshift_right", .{}),
            .bitshift_right_assign => writer.print("bitshift_right_assign", .{}),
            .lambda1 => writer.print("lambda1", .{}),
            .lambda2 => writer.print("lambda2", .{}),
            .lambda3 => writer.print("lambda3", .{}),
            .lambda4 => writer.print("lambda4", .{}),
            .lambda5 => writer.print("lambda5", .{}),
            .lambda6 => writer.print("lambda6", .{}),
            .lambda7 => writer.print("lambda7", .{}),
            .lambda8 => writer.print("lambda8", .{}),
            .indent => writer.print("indent", .{}),
            .access => writer.print("access", .{}),
            .readonly_member_access => writer.print("readonly_member_access", .{}),
            .writable_member_access => writer.print("writable_member_access", .{}),
            .temporary_member_access => writer.print("temporary_member_access", .{}),
            .wr_member_access => writer.print("wr_member_access", .{}),
            .rw_member_access => writer.print("rw_member_access", .{}),
            .wt_member_access => writer.print("wt_member_access", .{}),
            .tw_member_access => writer.print("tw_member_access", .{}),
            .rt_member_access => writer.print("rt_member_access", .{}),
            .tr_member_access => writer.print("tr_member_access", .{}),
            .rwt_member_access => writer.print("rwt_member_access", .{}),
            .wrt_member_access => writer.print("wrt_member_access", .{}),
            .rtw_member_access => writer.print("rtw_member_access", .{}),
            .wtr_member_access => writer.print("wtr_member_access", .{}),
            .trw_member_access => writer.print("trw_member_access", .{}),
            .twr_member_access => writer.print("twr_member_access", .{}),
        };
    }

    pub fn string(self: Self) SmallString {
        return switch (self) {
            .new_generic => SmallString.noAlloc("~"),
            .increment => SmallString.noAlloc("++"),
            .decrement => SmallString.noAlloc("--"),
            .assign => SmallString.noAlloc("="),
            .equals => SmallString.noAlloc("=="),
            .less_than => SmallString.noAlloc("<"),
            .less_equal => SmallString.noAlloc("<="),
            .greater_than => SmallString.noAlloc(">"),
            .greater_equal => SmallString.noAlloc(">="),
            .plus => SmallString.noAlloc("+"),
            .plus_assign => SmallString.noAlloc("+="),
            .minus => SmallString.noAlloc("-"),
            .minus_assign => SmallString.noAlloc("-="),
            .multiply => SmallString.noAlloc("*"),
            .multiply_assign => SmallString.noAlloc("*="),
            .exponentiate1 => SmallString.noAlloc("**"),
            .exponentiate1_assign => SmallString.noAlloc("**="),
            .exponentiate => SmallString.noAlloc("^"),
            .exponentiate_assign => SmallString.noAlloc("^="),
            .divide => SmallString.noAlloc("/"),
            .divide_assign => SmallString.noAlloc("/="),
            .integer_divide => SmallString.noAlloc("//"),
            .integer_divide_assign => SmallString.noAlloc("//="),
            .modulus => SmallString.noAlloc("%"),
            .modulus_assign => SmallString.noAlloc("%="),
            .remainder => SmallString.noAlloc("%%"),
            .remainder_assign => SmallString.noAlloc("%%="),
            .nullify => SmallString.noAlloc("?"),
            .nullish_or => SmallString.noAlloc("??"),
            .nullish_or_assign => SmallString.noAlloc("??="),
            .not => SmallString.noAlloc("!"),
            .not_not => SmallString.noAlloc("!!"),
            .not_equal => SmallString.noAlloc("!="),
            .declare_generic => SmallString.noAlloc("`"),
            .declare_readonly => SmallString.noAlloc(":"),
            .declare_writable => SmallString.noAlloc(";"),
            .declare_temporary => SmallString.noAlloc("."),
            .comma => SmallString.noAlloc(","),
            .logical_and => SmallString.noAlloc("&&"),
            .logical_and_assign => SmallString.noAlloc("&&="),
            .logical_or => SmallString.noAlloc("||"),
            .logical_or_assign => SmallString.noAlloc("||="),
            .bitwise_and => SmallString.noAlloc("&"),
            .bitwise_and_assign => SmallString.noAlloc("&="),
            .bitwise_or => SmallString.noAlloc("|"),
            .bitwise_or_assign => SmallString.noAlloc("|="),
            .bitwise_xor => SmallString.noAlloc("><"),
            .bitwise_xor_assign => SmallString.noAlloc("><="),
            .bitwise_flip => SmallString.noAlloc("<>"),
            .bitshift_left => SmallString.noAlloc("<<"),
            .bitshift_left_assign => SmallString.noAlloc("<<="),
            .bitshift_right => SmallString.noAlloc(">>"),
            .bitshift_right_assign => SmallString.noAlloc(">>="),
            .lambda1 => SmallString.noAlloc("$"),
            .lambda2 => SmallString.noAlloc("$$"),
            .lambda3 => SmallString.noAlloc("$$$"),
            .lambda4 => SmallString.noAlloc("$$$$"),
            .lambda5 => SmallString.noAlloc("$$$$$"),
            .lambda6 => SmallString.noAlloc("$$$$$$"),
            .lambda7 => SmallString.noAlloc("$$$$$$$"),
            .lambda8 => SmallString.noAlloc("$$$$$$$$"),
            .indent => SmallString.noAlloc("\t"),
            .access => SmallString.noAlloc(" "),
            .readonly_member_access => SmallString.noAlloc("::"),
            .writable_member_access => SmallString.noAlloc(";;"),
            .temporary_member_access => SmallString.noAlloc(".."),
            .wr_member_access => SmallString.noAlloc(";:"),
            .rw_member_access => SmallString.noAlloc(":;"),
            .wt_member_access => SmallString.noAlloc(";."),
            .tw_member_access => SmallString.noAlloc(".;"),
            .rt_member_access => SmallString.noAlloc(":."),
            .tr_member_access => SmallString.noAlloc(".:"),
            .rwt_member_access => SmallString.noAlloc(":;."),
            .wrt_member_access => SmallString.noAlloc(";:."),
            .rtw_member_access => SmallString.noAlloc(":.;"),
            .wtr_member_access => SmallString.noAlloc(";.:"),
            .trw_member_access => SmallString.noAlloc(".:;"),
            .twr_member_access => SmallString.noAlloc(".;:"),
            .none => SmallString{},
        };
    }

    pub fn isPrefixable(self: Self) bool {
        return switch (self) {
            .new_generic,
            .not,
            .not_not,
            .nullify,
            positive,
            .increment,
            negate,
            .decrement,
            .lambda1,
            .lambda2,
            .lambda3,
            .lambda4,
            .lambda5,
            .lambda6,
            .lambda7,
            .lambda8,
            .readonly_member_access,
            .writable_member_access,
            .temporary_member_access,
            .wr_member_access,
            .rw_member_access,
            .wt_member_access,
            .tw_member_access,
            .rt_member_access,
            .tr_member_access,
            .rwt_member_access,
            .wrt_member_access,
            .rtw_member_access,
            .wtr_member_access,
            .trw_member_access,
            .twr_member_access,
            => true,
            else => false,
        };
    }

    pub fn isPostfixable(self: Self) bool {
        return switch (self) {
            moot,
            cancel_if_null,
            .comma,
            .increment,
            .decrement,
            .declare_generic,
            .declare_readonly,
            .declare_writable,
            .declare_temporary,
            .wr_member_access,
            .rw_member_access,
            .wt_member_access,
            .tw_member_access,
            .rt_member_access,
            .tr_member_access,
            .rwt_member_access,
            .wrt_member_access,
            .rtw_member_access,
            .wtr_member_access,
            .trw_member_access,
            .twr_member_access,
            => true,
            else => false,
        };
    }

    pub fn isInfixable(self: Self) bool {
        return switch (self) {
            .assign,
            .equals,
            .less_than,
            .less_equal,
            .greater_than,
            .greater_equal,
            .plus,
            .plus_assign,
            .minus,
            .minus_assign,
            .multiply,
            .multiply_assign,
            .exponentiate1,
            .exponentiate1_assign,
            .exponentiate,
            .exponentiate_assign,
            .divide,
            .divide_assign,
            .integer_divide,
            .integer_divide_assign,
            .modulus,
            .modulus_assign,
            .remainder,
            .remainder_assign,
            .nullish_or,
            .nullish_or_assign,
            .not_equal,
            .declare_generic,
            .declare_readonly,
            .declare_writable,
            .declare_temporary,
            .comma,
            .indent,
            .access,
            .readonly_member_access,
            .writable_member_access,
            .temporary_member_access,
            .wr_member_access,
            .rw_member_access,
            .wt_member_access,
            .tw_member_access,
            .rt_member_access,
            .tr_member_access,
            .rwt_member_access,
            .wrt_member_access,
            .rtw_member_access,
            .wtr_member_access,
            .trw_member_access,
            .twr_member_access,
            .logical_and,
            .logical_and_assign,
            .logical_or,
            .logical_or_assign,
            .bitwise_and,
            .bitwise_and_assign,
            .bitwise_or,
            .bitwise_or_assign,
            .bitwise_xor,
            .bitwise_xor_assign,
            .bitwise_flip,
            .bitshift_left,
            .bitshift_left_assign,
            .bitshift_right,
            .bitshift_right_assign,
            => true,
            else => false,
        };
    }

    pub const positive: Self = .plus;
    pub const negate: Self = .minus;
    pub const negative: Self = .minus;
    pub const moot: Self = .not;
    pub const cancel_if_null: Self = .nullify;

    pub const Operation = OperatorOperation;
    const Self = @This();
};

const OperationType = enum {
    none,
    prefix,
    infix,
    postfix,

    pub fn printLine(self: Self, writer: anytype) !void {
        try self.print(writer);
        try writer.print("\n", .{});
    }

    pub fn print(self: Self, writer: anytype) !void {
        return switch (self) {
            .none => writer.print("Operation.Type.none", .{}),
            .prefix => writer.print("Operation.Type.prefix", .{}),
            .infix => writer.print("Operation.Type.infix", .{}),
            .postfix => writer.print("Operation.Type.postfix", .{}),
        };
    }

    const Self = @This();
};

const OperatorOperation = struct {
    operator: Operator = .none,
    type: Type = Type.none,

    pub fn printLine(self: Self, writer: anytype) !void {
        try self.print(writer);
        try writer.print("\n", .{});
    }

    pub fn print(self: Self, writer: anytype) !void {
        try writer.print("Operation{{ .operator = ", .{});
        try self.operator.print(writer);
        try writer.print(", .type = ", .{});
        try self.type.print(writer);
        try writer.print("}}", .{});
    }

    pub fn isPrefix(self: Self) bool {
        return self.type == Type.prefix;
    }
    pub fn isInfix(self: Self) bool {
        return self.type == Type.infix;
    }
    pub fn isPostfix(self: Self) bool {
        return self.type == Type.postfix;
    }

    /// Higher precedence means higher priority
    pub fn precedence(self: Self, compare: Compare) u8 {
        const rtl: u8 = @intFromEnum(compare);
        return switch (self.operator) {
            .none => 254,
            .new_generic => 250,
            .increment, .decrement => 220,
            .equals,
            .less_than,
            .less_equal,
            .greater_than,
            .greater_equal,
            .not_equal,
            => 160,
            .plus, .minus => if (self.isInfix()) 180 else (210 + rtl),
            .exponentiate1, .exponentiate => 220,
            .multiply,
            .divide,
            .integer_divide,
            .modulus,
            .remainder,
            => 190,
            .nullify => 230,
            // TODO: should this drop in priority???
            .nullish_or => 230,
            .not, .not_not => 210 + rtl,
            .comma => 10,
            .indent => 20,
            .logical_and => 170,
            .logical_or => 170,
            .bitwise_and, .bitwise_or => 180,
            .bitwise_xor => 170,
            .bitwise_flip => 210 + rtl,
            .bitshift_left, .bitshift_right => 200,
            .declare_generic,
            .declare_readonly,
            .declare_writable,
            .declare_temporary,
            => 130 + rtl,
            .assign,
            .plus_assign,
            .minus_assign,
            .multiply_assign,
            .exponentiate1_assign,
            .exponentiate_assign,
            .divide_assign,
            .integer_divide_assign,
            .modulus_assign,
            .remainder_assign,
            .nullish_or_assign,
            .logical_and_assign,
            .logical_or_assign,
            .bitwise_and_assign,
            .bitwise_or_assign,
            .bitwise_xor_assign,
            .bitshift_left_assign,
            .bitshift_right_assign,
            => 120 + rtl,
            .lambda1,
            .lambda2,
            .lambda3,
            .lambda4,
            .lambda5,
            .lambda6,
            .lambda7,
            .lambda8,
            => 250,
            .access,
            .readonly_member_access,
            .writable_member_access,
            .temporary_member_access,
            .wr_member_access,
            .rw_member_access,
            .wt_member_access,
            .tw_member_access,
            .rt_member_access,
            .tr_member_access,
            .rwt_member_access,
            .wrt_member_access,
            .rtw_member_access,
            .wtr_member_access,
            .trw_member_access,
            .twr_member_access,
            => 240,
        };
    }

    pub const Compare = enum {
        on_left,
        on_right,
    };
    pub const Type = OperationType;
    const Self = @This();
};

test "rewritten operator tokens" {
    try std.testing.expectEqual(Operator.declare_writable, Operator.init(";="));
    try std.testing.expectEqual(Operator.declare_readonly, Operator.init(":="));
}
